...
kappa_list = zeros(1, length(lx_list));
G_list = zeros(1, length(lx_list));
R2_list = zeros(1, length(lx_list));


for lx_index = 1:length(lx_list)
    lx = lx_list(lx_index);
    disp(lx);
    ...
    correlationObj = Chern_insulator_correlation_matrix_2d;
    ...
    kappa = G - sum_S_2;
    disp(kappa);
    kappa_list(lx_index) = kappa;  
end

%export the result to a csv file
file_name = sprintf("kappa_L_%d_%d_Chern_u_%.2f_PBC.csv", l_start, l_end, u);
data_matrix = transpose([lx_list; kappa_list; G_list; R2_list]);
